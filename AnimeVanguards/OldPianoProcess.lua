...
function waits.fors(path, timeout)
    local t = tick()
    repeat
        local obj = path()
...
local function UClick(btn)
    if not btn then
        return false
    end
    if btn:IsA("GuiButton") then
        -- Lấy vị trí và kích thước của nút
        ...
        -- Giả lập click chuột trái xuống và nhả ra
    ...
    elseif btn:IsA("ImageButton") then
        ...
        -- AnchorPoint correction
        local anchor = btn.AnchorPoint
        local x = pos.X + size.X * (0.5 - anchor.X)
        local y = pos.Y + size.Y * (0.5 - anchor.Y) + inset.Y
...
local function StartJamSession()
    ...
    -- OPEN EVENTS UI
    local HUD = waits.fors(function() return Gui:FindFirstChild("HUD") end)
    Click(HUD.RightButtons.Events.Button)
    waits.random(1, 1.5)

    print(HUD.RightButtons.Events.Button)

    local Holder = waits.fors(function() return Gui.Events.Holder end)
    ...
    if not PianoButton then
        Click(Filters.Regular.Button)
        waits.random(0.5, 0.75)
    ...
    -- JAM SESSION GUI
    local JamSessionGui = waits.fors(function() return Gui:FindFirstChild("JamSessionGui") end, 5)
            ...
            if HasMissing(ScriptConfig.Scores[songBtn.Name]) then
                ...
                for i = 1, 4 do
                    ...
                    if score < TARGET_SCORE then
                        Click(SongInfo.Play.Button)
                        return
                    else
                        ScriptConfig.Scores[songBtn.Name][diff] = score
                        Click(SongInfo.Difficulty.Button)
                        waits.random(2, 2.25)
                    end
                end
            end
    ...
    Click(JamSessionGui.Main.Cancel.Button);
end
